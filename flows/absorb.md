...
2. Ask which mode the user wants (they can also mix, per batch):
   - **Adjudicate now**: walk them through candidates in batches (~10 at a
     ...
     `TASKS add ...` in dependency order. Dropped candidates are simply
     never added — don't add them and then mark them `not-planned`; that
     status is for tasks the board already carried.
   - **Funnel to proposed**: for users without time to review everything in
     one sitting — add each plausible item as `TASKS add ... --status
     proposed` with your best-guess title/area/deps, pausing only for
     items too unclear to record faithfully. They queue in the review inbox
     (flows/review.md) for adjudication at the user's convenience; obvious
     junk (duplicates, clearly-done items) may be skipped, but say what was
     skipped and why.
3. Offer to retire the source file (delete, or replace its contents with a
